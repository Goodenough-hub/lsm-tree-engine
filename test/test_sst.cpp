...
protected:
    ...
    std::shared_ptr<SST> create_test_sst(size_t block_size, size_t num_entries)
    {
        ...
        auto block_cache = std::make_shared<BlockCache>(LSM_BLOCK_CACHE_CAPACITY,
                                                        LSM_BLOCK_CACHE_K);
...
TEST_F(SSTTest, BlockSplitting)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        std::string key = "key" + std::to_string(i);
        std::string value =
            std::string(100, 'v') + std::to_string(i); // 较大的value
        builder.add(key, value);
    }
...
//   SSTBuilder builder(4096, true); // 4KB blocks
//   auto block_cache =
//       std::make_shared<BlockCache>(LSM_BLOCK_CACHE_CAPACITY,
//       LSM_BLOCK_CACHE_K);

//   // 添加大量数据
//   for (int i = 0; i < 1000; i++) {
//     // key格式：key000, key001, ..., key999
//     std::string key = "key" + std::string(3 - std::to_string(i).length(),
//     '0') +
//                       std::to_string(i);
